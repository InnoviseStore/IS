...
const ProductCard = React.memo(function ProductCard({
  ...
  return (
    <div className="group relative flex flex-col rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-900/80 backdrop-blur-md border border-slate-200/80 dark:border-slate-800/80 shadow-md shadow-slate-200/30 dark:shadow-black/40 hover:shadow-2xl hover:border-blue-400 dark:hover:border-blue-600 transition-all duration-300 overflow-hidden">
      {/* Image with zoom on hover and Link to detailed page */}
      <Link href={productHref} prefetch={true} className="relative aspect-square rounded-t-2xl sm:rounded-t-3xl overflow-hidden bg-slate-100 dark:bg-slate-800 block cursor-pointer">
        {product.image_url ? (
          <Image
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-1.5 bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900">
            <Package className="h-8 w-8 sm:h-10 sm:w-10 text-slate-400 dark:text-slate-500" />
            <span className="text-[10px] sm:text-xs text-slate-400 dark:text-slate-500 font-medium">Sin imagen</span>
          </div>
        )}

        {/* Stock indicator badge */}
        <div className="absolute top-2 left-2 sm:top-3 sm:left-3 flex items-center gap-1 sm:gap-1.5 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full bg-white/95 dark:bg-slate-900/95 backdrop-blur-md shadow-xs text-[10px] sm:text-xs font-semibold text-slate-800 dark:text-slate-200">
          <span className={`h-1.5 w-1.5 sm:h-2 sm:w-2 rounded-full ${stockColor(product.stock_quantity)}`} />
          <span>{stockLabel(product.stock_quantity)}</span>
        </div>

        {/* Quick view icon badge */}
        <div className="absolute top-2 right-2 sm:top-3 sm:right-3 p-1.5 sm:p-2 rounded-full bg-black/40 text-white backdrop-blur-xs opacity-0 group-hover:opacity-100 transition-opacity duration-200 hover:bg-black/60">
      ...
      <div className="flex flex-col flex-1 p-2.5 sm:p-4 gap-1.5 sm:gap-2">
        <div>
          <span className="text-[9px] sm:text-[10px] font-extrabold uppercase tracking-wider text-blue-600 dark:text-blue-400">
            {detectCategory(product.name, product.description)}
          </span>
          <Link href={productHref} prefetch={true}>
            <h3 className="font-bold text-xs sm:text-sm text-slate-900 dark:text-white leading-snug line-clamp-2 hover:text-blue-600 dark:hover:text-blue-400 transition-colors mt-0.5">
              {product.name}
            </h3>
          </Link>
          {product.sku && (
            <p className="text-[10px] font-mono text-slate-500 dark:text-slate-400 mt-0.5 truncate">SKU: {product.sku}</p>
          )}
          ...
          {apparel && (apparel.gender || (apparel.sizes && apparel.sizes.length > 0)) && (
            <div className="flex items-center gap-1 flex-wrap mt-1">
              {apparel.gender && (
                <span className="text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded-md bg-purple-50 text-purple-700 dark:bg-purple-950/80 dark:text-purple-300 border border-purple-200 dark:border-purple-800">
                  {apparel.gender}
                </span>
              )}
              {apparel.sizes && apparel.sizes.length > 0 && (
                <span className="text-[9px] sm:text-[10px] font-semibold px-1.5 py-0.5 rounded-md bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
        ...
        {cleanDescription && (
          <p className="hidden xs:block text-xs text-slate-600 dark:text-slate-300 line-clamp-2 leading-relaxed">
            {cleanDescription}
          </p>
        )}
        ...
        <div className="mt-auto pt-2 border-t border-slate-100 dark:border-slate-800">
          <p className="text-blue-600 dark:text-blue-400 font-extrabold text-base sm:text-xl leading-none tracking-tight">
            ${formatUsd(product.unit_price_usd)}
            <span className="text-slate-500 dark:text-slate-400 font-normal text-[10px] sm:text-xs ml-1">USD</span>
          </p>
          <p className="text-slate-700 dark:text-slate-300 text-[11px] sm:text-xs font-semibold mt-1">
        ...
        <div className="pt-1.5 sm:pt-2">
          {outOfStock ? (
            <button
              disabled
              className="w-full py-2 sm:py-2.5 px-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 text-xs font-semibold cursor-not-allowed text-center"
            >
              Agotado
            </button>
          ) : cartItem ? (
            /* In-cart quantity controls */
            <div className="flex items-center justify-between rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-1 shadow-2xs">
              <button
                onClick={onDecrease}
                className="h-7 w-7 sm:h-8 sm:w-8 rounded-lg flex items-center justify-center text-slate-700 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-700 transition active:scale-90"
                aria-label="Disminuir cantidad"
              >
                <Minus className="h-3.5 w-3.5" />
              </button>
              <span className="text-xs font-bold text-slate-900 dark:text-white px-1 truncate">
                {cartItem.quantity}
              </span>
              <button
                onClick={onIncrease}
                ...
                className="h-7 w-7 sm:h-8 sm:w-8 rounded-lg flex items-center justify-center text-slate-700 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-700 disabled:opacity-40 transition active:scale-90"
            ...
            <button
              onClick={handleAdd}
              className={`w-full min-h-[38px] sm:min-h-[42px] flex items-center justify-center gap-1.5 py-2 sm:py-2.5 px-2 sm:px-3 rounded-xl text-xs font-bold transition-all duration-200 shadow-xs active:scale-95 cursor-pointer ${
                justAdded
      ...
      <div className="p-3.5 flex flex-col flex-1 justify-between gap-2.5 bg-slate-900/95">
        ...
        <div className="pt-2 border-t border-slate-800">
          <div className="flex items-baseline justify-between">
            <p className="text-sm sm:text-base font-extrabold text-cyan-400 tracking-tight" style={{ color: primaryColor }}>
              ${formatUsd(product.unit_price_usd)}
          ...
          <div className="mt-2">
            {outOfStock ? (
              <button disabled className="w-full py-1.5 sm:py-2 rounded-xl bg-slate-800 text-slate-500 text-xs font-mono">
                [AGOTADO]
              </button>
            ) : cartItem ? (
              ...
              <button
                onClick={handleAdd}
                className="w-full py-2 rounded-xl text-xs font-bold text-slate-950 transition active:scale-95 shadow-md shadow-cyan-500/20 cursor-pointer truncate px-2"
                style={{ backgroundColor: primaryColor }}
              >
                {justAdded ? '✓ Listo' : '+ Añadir'}
              </button>
            )}
          </div>
        </div>
...
const BoutiqueCard = React.memo(function BoutiqueCard({
  ...
  return (
    <div className="group flex flex-col bg-white/90 dark:bg-[#1f1c1a]/90 rounded-2xl sm:rounded-3xl border border-amber-200/60 dark:border-amber-900/40 p-2.5 sm:p-3.5 shadow-md shadow-amber-950/5 hover:shadow-xl hover:border-amber-400 transition-all duration-300">
      <Link href={productHref} prefetch={true} className="relative aspect-square rounded-xl sm:rounded-2xl overflow-hidden bg-[#faf7f2] dark:bg-[#141210] block mb-2 sm:mb-3">
        {product.image_url ? (
          <Image
            src={product.image_url}
            alt={product.name}
        ...
        <div className="absolute top-2 right-2 sm:top-2.5 sm:right-2.5 px-2 py-0.5 rounded-full bg-amber-50/90 dark:bg-amber-950/90 text-amber-800 dark:text-amber-200 border border-amber-300/50 text-[9px] sm:text-[10px] font-semibold">
          {stockLabel(product.stock_quantity)}
        </div>
      </Link>

      <div className="flex flex-col flex-1 justify-between gap-1.5 sm:gap-2">
        <div>
          <span className="text-[9px] sm:text-[10px] font-medium tracking-wider text-amber-700 dark:text-amber-400 uppercase">
            {detectCategory(product.name, product.description)}
          </span>
          <Link href={productHref} prefetch={true}>
            <h3 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-slate-100 line-clamp-2 hover:text-amber-700 transition-colors mt-0.5 leading-snug">
              {product.name}
            </h3>
          </Link>
        </div>

        <div className="pt-2 border-t border-amber-100 dark:border-amber-900/30 flex items-center justify-between gap-1">
          <div>
            <p className="text-sm sm:text-base font-extrabold text-amber-800 dark:text-amber-300" style={{ color: primaryColor }}>
              ${formatUsd(product.unit_price_usd)}
            </p>
          ...
          <div>
            {outOfStock ? (
              ...
              <div className="flex items-center gap-1 bg-amber-50 dark:bg-amber-950/60 px-1.5 py-1 rounded-xl border border-amber-200 dark:border-amber-800">
              ...
              <button
                onClick={handleAdd}
                className="px-2.5 sm:px-3.5 py-1.5 rounded-xl text-xs font-bold text-white shadow-xs transition active:scale-95 cursor-pointer truncate"
                style={{ backgroundColor: primaryColor }}
              >
                {justAdded ? '✓ Listo' : 'Elegir'}
              </button>
            )}
          </div>
        </div>
...
export default function ProductGrid({
  ...
  return (
    <section className="space-y-6 pt-4">
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 sm:gap-3 pt-1">
          <div className="flex items-center gap-1.5 sm:gap-2">
            <span className="text-[11px] sm:text-xs text-slate-500 font-medium">Precio:</span>
            <select
              value={priceRange}
              onChange={(e) => setPriceRange(e.target.value)}
              className="px-2.5 py-1.5 rounded-xl text-[11px] sm:text-xs font-semibold bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200 outline-none cursor-pointer"
            >
            ...
            {showViewToggle && (
              <div className="flex items-center p-0.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/90 shadow-2xs">
                <button
                  ...
                  className={`p-1.5 sm:px-2.5 sm:py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition cursor-pointer ${
                    viewMode === 'grid'
                      ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-900 shadow-xs'
      ...
      {itemCount > 0 && (
        <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-30 sm:hidden w-fit pointer-events-auto">
          <button
            onClick={openCart}
            className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 text-white font-black text-xs shadow-xl shadow-blue-500/35 hover:opacity-95 active:scale-95 transition-all border border-white/20 backdrop-blur-md"
          >
