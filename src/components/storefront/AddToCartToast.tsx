...
export default function AddToCartToast({ exchangeRate = 1 }: { exchangeRate?: number }) {
  ...
  return (
    <div
      className={`fixed bottom-5 right-4 sm:right-6 z-50 max-w-sm sm:max-w-md w-[calc(100vw-2rem)] transition-all duration-300 ease-out ${
        isVisible
      ...
      aria-live="polite"
    >
      <div className="glass-card p-3.5 sm:p-4 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border border-emerald-500/30 dark:border-emerald-500/30 shadow-2xl shadow-emerald-500/10 dark:shadow-black/60 rounded-2xl sm:rounded-3xl flex items-center gap-3 relative overflow-hidden">
        ...
        {/* Product Image / Icon */}
        <div className="relative h-12 w-12 rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-800 shrink-0 border border-slate-200 dark:border-slate-700 flex items-center justify-center">
          {item.image_url ? (
            <Image
              src={item.image_url}
              alt={item.name}
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button
            ...
            className="flex items-center gap-1 px-3 py-2 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white text-xs font-bold shadow-md shadow-blue-500/20 active:scale-95 transition cursor-pointer"
          >
